...
(function () {
  ...
  // ------------------------------------------------------------------
  // Contact form — layered antispam
  // Layers (all client-side; static-site compatible):
  //   1. Honeypot field (.hp input) — bots fill it, humans don't.
  //   2. Time-trap — minimum dwell time on page before submit allowed.
  //   3. JS-required submit — form has no native action; it's set by JS.
  //   4. Submit token — random nonce stored in sessionStorage at load,
  //      required to submit, single-use.
  //   5. Human-check question — user must answer a small word puzzle.
  //   6. Mailto dispatch — final dispatch is via mailto: with rot13'd
  //      address; bots scraping the HTML won't see a usable email.
  //   7. Hook for Turnstile/hCaptcha — placeholder for when a server
  //      backend is wired in (see comments below).
  // ------------------------------------------------------------------
  function initContactForm() {
    ...
    // Per-load submit token
    var token = "tk_" + Math.random().toString(36).slice(2) + Date.now().toString(36);
    try { sessionStorage.setItem("contact_token", token); } catch (_e) {}
    var tokenInput = form.querySelector('input[name="form_token"]');
    if (tokenInput) tokenInput.value = token;

    // Per-load human check
    var checks = [
      { q: "What color is the sky on a clear day?", a: ["blue"] },
      { q: "Type the word 'human'.", a: ["human"] },
      { q: "How many letters are in DNS?", a: ["3", "three"] },
      { q: "Is fire hot or cold?", a: ["hot"] },
      { q: "Which is larger, 10 or 2?", a: ["10", "ten"] }
    ];
    var pick = checks[Math.floor(Math.random() * checks.length)];
    var qLabel = form.querySelector("[data-human-question]");
    if (qLabel) qLabel.textContent = pick.q;
    form.dataset.humanAnswers = JSON.stringify(pick.a);

    // Optional Turnstile widget
    var turnstileSiteKey = form.getAttribute("data-turnstile-sitekey") || "";
    var turnstileWidgetId = null;
    var turnstileMount = $("turnstile-mount");
    if (turnstileSiteKey && turnstileMount && turnstileMount.getAttribute("data-rendered") !== "true") {
      ...
      window.__onTurnstileLoad = function () {
        ...
        turnstileWidgetId = window.turnstile.render("#turnstile-mount", {
          sitekey: turnstileSiteKey,
          callback: function (tok) {
            var hidden = form.querySelector('input[name="turnstileToken"]');
            if (hidden) hidden.value = tok || "";
          },
        });
        turnstileMount.setAttribute("data-rendered", "true");
      };
    ...
    function setStatus(msg, level) {
      ...
      if (level === "bad") statusEl.classList.add("bad");
    }
    ...
    form.addEventListener("submit", async function (e) {
      ...
      // Layer 1: honeypot
      var hp = form.querySelector('input[name="companyWebsite"]');
      if (hp && hp.value) {
        // Silent success so bots don't learn what tripped them.
        setStatus("Thanks — your message has been queued.", "ok");
        return;
      }

      // Layer 2: time-trap
      if (Date.now() - loadedAt < minDwellMs) {
        setStatus("Please take a moment to review your message before sending.", "bad");
        return;
      }

      // Layer 3+4: token must match what we stored
      var stored = "";
      try { stored = sessionStorage.getItem("contact_token") || ""; } catch (_e) {}
      var sent = (tokenInput ? tokenInput.value : "");
      if (!sent || sent !== stored || stored.indexOf("tk_") !== 0) {
        setStatus("Session expired. Refresh the page and try again.", "bad");
        return;
      }

      // Layer 5: human check
      var answer = (form.querySelector('input[name="human_check"]') || {}).value || "";
      var expected = JSON.parse(form.dataset.humanAnswers || "[]");
      var normalized = String(answer).trim().toLowerCase();
      if (!expected.some(function (a) { return a.toLowerCase() === normalized; })) {
        setStatus("That answer doesn't look right. Try again.", "bad");
        return;
      }
      ...
      var payload = {
        ...
        companyWebsite: "",
        turnstileToken: (form.querySelector('input[name="turnstileToken"]') || {}).value || "",
      };

      // Single-use: invalidate token now
      try { sessionStorage.removeItem("contact_token"); } catch (_e) {}
      ...
      try {
        ...
        if (resp.ok) {
          setStatus(data.message || "Your inquiry was sent successfully.", "ok");
          form.reset();
        ...
        if (data.fieldErrors) {
          Object.keys(data.fieldErrors).forEach(function (k) { setFieldError(k, data.fieldErrors[k]); });
        }

        var msg = data.error || ("Submission failed (HTTP " + resp.status + ").");
        if (data.code === "CONTACT_NOT_CONFIGURED") {
          msg = "Contact delivery isn't configured on this environment yet. Please email us directly.";
        } else if (data.code === "RATE_LIMITED") {
          msg = "Too many submissions. Please wait a few minutes and try again.";
        } else if (data.code === "ABUSE_DETECTED") {
